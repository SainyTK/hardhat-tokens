...
const mint = async (contractName: string, account: string, amount: BigNumberish) => {
    const [owner] = await ethers.getSigners();

    const addressList = await addressUtils.getAddressList(hre.network.name);
    const token = await MintableToken__factory.connect(addressList[contractName], owner);
...
async function main() {
    ...
    await mint('KUSDT', address, parseEther('200000'));
