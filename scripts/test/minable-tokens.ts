...
async function main() {
  ...
  const kusdt = await MinableToken.deploy(
    "Bitkub-Peg USDT",
    "KUSDT",
    kusdtFaucetRate,
    maxElapse,
  ...
  console.log(
    "KUSDT permission: ",
    await counter.counterRights(kusdt.address).then((res) => res.toString())
  );

  const kbtcFaucetRate = parseEther("0.0001");
  const kbtc = await MinableToken.deploy(
    "Bitkub-Peg BTC",
    "KBTC",
    kbtcFaucetRate,
    maxElapse,
  ...
  console.log(
    "KBTC permission: ",
    await counter.counterRights(kbtc.address).then((res) => res.toString())
  );
  ...
  for (let i = 0; i < round; i++) {
    ...
    console.log(
      "Elapse time: ",
      await counter
        .getElapsedTimeOf(owner.address)
  ...
  console.log(
    "Minable KUSDT: ",
    await kusdt.getMiningReward().then((res) => formatEther(res))
  );
  console.log(
    "Minable KBTC: ",
    await kbtc.getMiningReward().then((res) => formatEther(res))
  );
  ...
  console.log(
    "KBTC balance: ",
    await kbtc.balanceOf(owner.address).then((res) => formatEther(res))
  );
